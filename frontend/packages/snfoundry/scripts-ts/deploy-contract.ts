...
import { CompiledSierra, LegacyContractClass, RawArgs } from "starknet";
...
const deployContract = async (
  ...
  try {
    if (!existingClassHash) {
      const { suggestedMaxFee: estimatedFeeDeclare } = await deployer
        .estimateDeclareFee(
          {
      ...
      const { suggestedMaxFee: estimatedFeeDeploy } = await deployer
        .estimateDeployFee({
  ...
  try {
    ...
    contractAddress = "0x" +
      tryDeclareAndDeploy.deploy.address.slice(2).padStart(64, "0");
  } catch (e) {
    console.log("Error", e);
  }
...
export { deployContract, deployer, exportDeployments, provider };
